/// So sánh signal bar-by-bar giữa 2 cách implement cùng 1 strategy:
...
fn print_table(title: &str, a: &[TaggedSignal], b: &[TaggedSignal]) {
    println!("\n┌─ {title} ─────────────────────────────────────────────────────────");
    println!("│  {:>6}  {:<12}  {:<12}", "bar", "hardcoded", "dynamic");
    println!("├───────────────────────────────────────────────────────────────────");

    // Merge all bar indices that produced any signal
    let mut idxs: Vec<usize> = a.iter().chain(b.iter())
        .map(|s| s.bar_idx)
        .collect();
    idxs.sort_unstable();
    idxs.dedup();
    ...
    for idx in &idxs {
        ...
        println!("│  {:>6}  {:<12}  {:<12}", idx, fmt(a), fmt(b));
    }

    // Parity check
    let a_key: Vec<_> = a.iter().map(|s| (s.bar_idx, s.dir)).collect();
    let b_key: Vec<_> = b.iter().map(|s| (s.bar_idx, s.dir)).collect();
    let parity = if a_key == b_key { "✓ identical" } else { "✗ mismatch!" };
    println!("└─ signals: hardcoded={} dynamic={}  {parity}",
        a.len(), b.len());
}
