...
use crate::types::{
    BacktestRequest, BacktestResponse, BuyHoldBenchmarkResponse, CurvePoint, ExitLevel,
    RegimeSummaryResponse, TradeResponse,
};
...
pub fn run(req: BacktestRequest, data_dir: &Path) -> Result<BacktestResponse> {
    ...
    // Sizer priority: position_size_quantity > position_size_usd > position_size_pct
    ...
    let exit_rules = req.exit.map(|cfg| exit_rules_from_config(cfg, &params))
        .unwrap_or_default();
    ...
    // Collect bars for benchmark (feed is exhausted — reset and drain)
    feed.reset();
    ...
    let exposure_pct = {
        let total_ms = engine.portfolio.equity_curve.last()
            .and_then(|last| engine.portfolio.equity_curve.first().map(|first| last.timestamp - first.timestamp))
            .unwrap_or(0);
...
/// ATR-expression levels (`"N*atr(P)"`) → injected into `cel_params` so that
/// `CelStrategy` can handle them internally; engine-level ExitRules gets `None`
/// for those fields (engine doesn't know ATR at construction time).
pub fn exit_rules_from_config(
    cfg: crate::types::ExitConfig,
    cel_params: &serde_json::Value,
) -> ExitRules {
...
/// Called by the CEL `From` conversion so that `CelStrategy::from_params` picks them up.
pub fn inject_atr_exit_into_cel_params(
    ...
    if let Some(level) = &cfg.tp {
        match level {
            ExitLevel::Pct(v)  => { params.insert("tp".into(), json!(v)); }
            ExitLevel::Expr(_) => {
    ...
    if let Some(level) = &cfg.sl {
        match level {
            ExitLevel::Pct(v)  => { params.insert("sl".into(), json!(v)); }
            ExitLevel::Expr(_) => {
                if let Some((mult, period)) = level.as_atr() {
