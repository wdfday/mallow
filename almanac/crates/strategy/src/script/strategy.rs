...
pub use super::lint::{
    script_lint, DeclaredIndicator, LintDiagnostic, ScriptLintScope, KNOWN_INDICATOR_TYPES,
};
...
"#;
    ...
    fn compile_mtf_script() {
        ScriptStrategy::from_script(MTF_SCRIPT).expect("MTF script should compile");
    }
    ...
    fn mtf_runs_200_m1_bars_no_panic() {
        let mut s = ScriptStrategy::from_script(MTF_SCRIPT).unwrap();
        for i in 0..200 { let _ = s.on_bar(&make_bar(i)); }
    }
