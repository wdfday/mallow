...
"#;
    ...
    assert_parity("script EMA(10,50) vs MaCrossover(10,50)", &named_sigs, &script_sigs);
}

// ── RSI threshold: Script ───────────────────────────────────────────────────────
...
"#;
    ...
    assert!(!script_sigs.is_empty(), "script_rsi: must produce signals");
}

// ── TP/SL: Script ──────────────────────────────────────────────────────────────

/// ATR-based TP and SL: Script emits Long signals with correct `target_price`
...
"#;
    let mut script_strat = build_strategy("script", &json!({ "script": script_src })).unwrap();
    let script_full = run_full(script_strat.as_mut(), &bars);

    let entries: Vec<&Signal> = script_full.iter().filter(|s| s.direction == Direction::Long).collect();
    assert!(!entries.is_empty(), "script_tp_sl: must produce Long signals");
...
/// Fixed-% TP: after a Long, the Script signal carries `target_price` at
...
"#;
    let mut script_strat = build_strategy("script", &json!({ "script": script })).unwrap();
    let sigs = run_full(script_strat.as_mut(), &bars);

    let entry_sigs: Vec<&Signal> = sigs.iter().filter(|s| s.direction == Direction::Long).collect();
    assert!(!entry_sigs.is_empty(), "script_fixed_pct: must produce Long signals");
...
"#;
    ...
    let r1 = run_sigs(script_strat.as_mut(), &bars);
    script_strat.reset();
    let r2 = run_sigs(script_strat.as_mut(), &bars);

    assert!(!r1.is_empty(), "script_reset: must produce signals");
    assert_parity("script reset: run1 vs run2", &r1, &r2);
}
...
"#;
    ...
    let r1 = run_full(script_strat.as_mut(), &bars);
    script_strat.reset();
    let r2 = run_full(script_strat.as_mut(), &bars);
    ...
    assert_parity("script tp/sl reset: directions", &d1, &d2);
