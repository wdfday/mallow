...
type Config struct {
	...
	// MaxMarginRatioPct caps the exchange-reported account-level maintenance
	// margin ratio (approaches 1.0 near liquidation) for margin/futures
	// accounts. New entries are rejected once portfolio.MarginRatio() ≥ this
	// value. 0 disables the gate — this is also the correct behavior for spot
	// accounts and any exchange that doesn't report a margin ratio, since
	// portfolio.MarginRatio() reads back zero ("no data") in that case, same
	// opt-in shape as MaxGrossExposurePct.
	MaxMarginRatioPct float64 `json:"max_margin_ratio_pct"`
}
