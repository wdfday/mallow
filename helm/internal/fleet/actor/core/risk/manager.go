...
func (m *Manager) Validate(intent strategy.Intent, handID string) (bool, string) {
	...
	// Gate 5: margin-ratio ceiling (entries only). 0 = disabled, and a zero
	// MarginRatio() reads as "no data" — spot accounts and exchanges that
	// don't report one never trip this gate regardless of config.
	if intent.Action.IsEntry() && m.cfg.MaxMarginRatioPct > 0 {
		if ratio := m.portfolio.MarginRatio(); ratio.IsPositive() {
			if mr, _ := ratio.Float64(); mr >= m.cfg.MaxMarginRatioPct {
				slog.Warn("risk: margin ratio ceiling reached",
					"margin_ratio", mr,
					"limit_pct", m.cfg.MaxMarginRatioPct,
				)
				return false, "margin ratio ceiling reached"
			}
		}
	}
...
func (m *Manager) UpdateConfig(cfg Config) {
	...
	slog.Info("risk: config updated",
		...
		"max_margin_ratio_pct", cfg.MaxMarginRatioPct,
	)
}
