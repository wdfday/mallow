...
// ── Margin ratio ceiling ──────────────────────────────────────────────────────

func TestValidate_MarginRatioCeiling_BlocksAboveThreshold(t *testing.T) {
	pf := newPort(10_000)
	m := risk.New(risk.Config{MaxMarginRatioPct: 0.8, MaxDrawdownPct: 1.0, DailyLossLimitPct: 1.0}, pf)

	pf.ApplySync(decimal.NewFromInt(10_000), nil, decimal.Zero, nil, decimal.NewFromFloat(0.85))

	if ok, reason := m.Validate(entryIntent("BTCUSDT", 1.0), "h1"); ok {
		t.Fatal("margin ratio above threshold should block new entry")
	} else if reason != "margin ratio ceiling reached" {
		t.Errorf("expected margin-ratio rejection reason, got %q", reason)
	}
	// Exit always passes regardless of margin ratio.
	if ok, reason := m.Validate(closeIntent("BTCUSDT"), "h1"); !ok {
		t.Fatalf("exit must always pass, got blocked: %s", reason)
	}
}

func TestValidate_MarginRatioCeiling_AllowsBelowThreshold(t *testing.T) {
	pf := newPort(10_000)
	m := risk.New(risk.Config{MaxMarginRatioPct: 0.8, MaxDrawdownPct: 1.0, DailyLossLimitPct: 1.0}, pf)

	pf.ApplySync(decimal.NewFromInt(10_000), nil, decimal.Zero, nil, decimal.NewFromFloat(0.5))

	if ok, reason := m.Validate(entryIntent("BTCUSDT", 1.0), "h1"); !ok {
		t.Fatalf("margin ratio below threshold should allow entry, got blocked: %s", reason)
	}
}

func TestValidate_MarginRatioCeiling_DisabledWhenNoData(t *testing.T) {
	pf := newPort(10_000)
	// Threshold configured, but this account never reported a margin ratio
	// (spot exchange, or no sync yet) — MarginRatio() reads back zero, which
	// must mean "no data", not "0% margin used".
	m := risk.New(risk.Config{MaxMarginRatioPct: 0.8, MaxDrawdownPct: 1.0, DailyLossLimitPct: 1.0}, pf)

	if ok, reason := m.Validate(entryIntent("BTCUSDT", 1.0), "h1"); !ok {
		t.Fatalf("no margin data should never trip the gate, got blocked: %s", reason)
	}
}

func TestValidate_MarginRatioCeiling_ZeroDisablesGate(t *testing.T) {
	pf := newPort(10_000)
	m := risk.New(risk.Config{MaxMarginRatioPct: 0, MaxDrawdownPct: 1.0, DailyLossLimitPct: 1.0}, pf)

	pf.ApplySync(decimal.NewFromInt(10_000), nil, decimal.Zero, nil, decimal.NewFromFloat(0.99))

	if ok, reason := m.Validate(entryIntent("BTCUSDT", 1.0), "h1"); !ok {
		t.Fatalf("MaxMarginRatioPct=0 should disable the gate entirely, got blocked: %s", reason)
	}
}
