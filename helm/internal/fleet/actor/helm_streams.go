...
func (r *HelmRuntime) connectStream(streamCtx context.Context) bool {
	...
	if err := streamer.StreamOrders(
		...
		r.handleRiskEvent,
		func(reason string) { r.TriggerAccountError(exchange.ErrClassAuth, reason) },
	); err != nil {
...
// handleBalanceEvent syncs portfolio state from a live balance push. USDT is
// the quote currency — SyncCash keeps it authoritative for the trading hot
// path (sizing, risk gates). Every asset (including USDT) also lands in the
// read-only Balances() side-channel via SyncBalance, so the full live wallet
// is visible, not just the quote asset.
func (r *HelmRuntime) handleBalanceEvent(ev exchange.BalanceEvent) {
	r.Portfolio.SyncBalance(ev.Asset, ev.Free)
	if ev.Asset != "USDT" {
		return
	}
...
// handlePositionEvent reconciles the helm's live position with an
// authoritative WS position push — Qty/AvgPrice/UnrealizedPnL are overwritten
// from the exchange's own reported values (Size is already signed by the
// adapter: positive=long, negative=short). A zero Size is logged rather than
// closing the position — closing goes through the audit-tracked paths
// (poslog KindPositionClosed, the startup reconciler's external-close
// handling), not this live-push path, so no Trade record / hand notification
// is silently skipped.
func (r *HelmRuntime) handlePositionEvent(ev exchange.PositionEvent) {
	...
	if skipped := r.Portfolio.SyncLivePosition(ev.Symbol, ev.Size, ev.EntryPrice, ev.UnrealizedPnL, ev.At); skipped {
		slog.Warn("position update: exchange reports flat — awaiting formal close via reconciler",
			"helm_id", r.HelmID,
			"symbol", ev.Symbol,
		)
	}
...
// handleRiskEvent handles margin call / liquidation warnings — logs and
// forwards to AccountEvents.OnMarginCall so the broker layer can notify the
// user. Does not pause the helm itself; unlike a credential/connection error,
// a margin warning isn't necessarily "stop trading now" — the handler decides.
func (r *HelmRuntime) handleRiskEvent(ev exchange.RiskEvent) {
	...
	if r.AccountEvents != nil {
		go r.AccountEvents.OnMarginCall(r.AccountID, ev)
	}
}
