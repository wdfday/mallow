...
// isLotSizeError reports whether err is a persistent sizing constraint — lot
// size or price filter — that will recur on every entry at the current
// configured quantity, regardless of market conditions. Classified via the
// exchange adapter's own ErrorClassifier (falling back to ClassifyGeneric)
// instead of a bespoke keyword heuristic — min-notional/min-quantity rejects
// are already grouped under ErrClassLotSize by every adapter's classifier
// (see docs/exchange-error-codes/), so no separate keyword list is needed.
func isLotSizeError(ex exchange.Exchange, err error) bool {
	if err == nil {
		return false
	}
