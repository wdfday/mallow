...
func buildReconcileRuntime(env *reconcileEnv, helmID uuid.UUID, capital decimal.Decimal) *actor.HelmRuntime {
	pf := portfolio.New(capital)
	rm := risk.New(risk.DefaultConfig(), pf)
	rt := actor.NewHelmRuntime(helmID, uuid.New(), uuid.New(), env.ex.Name(), pf, rm, env.ex, env.creds, nil, time.Now())
	rt.PosLog = env.pl
	return rt
}
