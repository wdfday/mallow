...
type DispatchStats struct {
	...
	RouteNoHand int64 // signals where hand_id was not found in the resolved actor.HelmRuntime
}
...
// SetDispatcher wires the dispatcher.SignalDispatcher so its NATS-level counters can be
// exported via NATSStats. Called from the app lifecycle after startup.
func (r *Registry) SetDispatcher(d *dispatcher.SignalDispatcher) {
	r.mu.Lock()
	r.dispatcher = d
	r.mu.Unlock()
}
