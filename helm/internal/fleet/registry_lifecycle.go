...
// Spawn creates and registers a actor.HelmRuntime for the given config.
...
func (r *Registry) Spawn(cfg *helmdomain.Helm, exchCfg helmdomain.ExchangeConfig) error {
	...
	rt := actor.NewHelmRuntime(cfg.ID, cfg.AccountID, cfg.UserID, brokerType, pf, riskMgr, ex, creds, cfg.LastSyncedAt, cfg.CreatedAt)
	rt.Herald = r.herald
	...
	// Wire the account-event handler so account/connection-level conditions
	// (credential rejection, sustained errors, margin calls, trading
	// restrictions) propagate to the broker service.
	r.mu.RLock()
	rt.AccountEvents = r.accountEvents
	r.mu.RUnlock()
...
// RotateCreds updates the credentials of a running actor.HelmRuntime in-place and
...
// Teardown stops and removes the actor.HelmRuntime for the given helm.
...
// Each actor.HelmRuntime owns its own fill streaming goroutines — see helm_fills.go.
func (r *Registry) StartStreaming(ctx context.Context) {
	r.mu.RLock()
	rts := make([]*actor.HelmRuntime, 0, len(r.helmRuntimes))
