...
type Signal struct {
	...
	// Used by the Tactician for volatility-based position sizing.
	ATR decimal.Decimal

	// PatternKind is the technical pattern that triggered this signal (e.g. "bull_flag").
	// Empty when not generated by a pattern detector.
	PatternKind string

	// PatternConfidence is the pattern detector's match confidence [0, 1].
	// Distinct from Strength (the position-sizing factor). Zero when PatternKind is empty.
