...
type Account struct {
	...
	// Balance — interpretation depends on account_type:
	//   spot:           available cash
	//   futures/unified: available margin (not yet used)
	CurrentBalance   decimal.Decimal  `gorm:"type:decimal(15,2);not null;default:0;column:current_balance" json:"current_balance"`
	AvailableBalance *decimal.Decimal `gorm:"type:decimal(15,2);column:available_balance" json:"available_balance,omitempty"`

	// Futures/unified only — zero for spot accounts
	Equity     decimal.Decimal `gorm:"type:decimal(15,2);not null;default:0;column:equity" json:"equity"`
	MarginUsed decimal.Decimal `gorm:"type:decimal(15,2);not null;default:0;column:margin_used" json:"margin_used"`
