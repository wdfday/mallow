...
func TestAddQuantity_FirstBuy(t *testing.T) {
	p := newPosition()
	p.AddQuantity(10, 100.0)

	assert.InDelta(t, 10.0, p.Quantity, delta)
	assert.InDelta(t, 100.0, p.AvgCost, delta)
	assert.InDelta(t, 1000.0, p.TotalCost, delta)
}
...
func TestAddQuantity_SecondBuyRecalculatesAvgCost(t *testing.T) {
	p := newPosition()
	p.AddQuantity(10, 100.0) // cost = 1000, qty = 10
	p.AddQuantity(10, 200.0) // cost = 2000, qty = 10 → total cost = 3000, qty = 20

	assert.InDelta(t, 20.0, p.Quantity, delta)
	assert.InDelta(t, 3000.0, p.TotalCost, delta)
	assert.InDelta(t, 150.0, p.AvgCost, delta) // (1000 + 2000) / 20 = 150
}

// 3. AddQuantity() multiple buys: running avg cost stays correct.
func TestAddQuantity_MultipleBuysRunningAvgCost(t *testing.T) {
	p := newPosition()
	p.AddQuantity(5, 100.0)  // cost 500,  qty 5,  avg 100
	p.AddQuantity(5, 120.0)  // cost 600,  qty 5,  total cost 1100, qty 10, avg 110
	p.AddQuantity(10, 110.0) // cost 1100, qty 10, total cost 2200, qty 20, avg 110

	assert.InDelta(t, 20.0, p.Quantity, delta)
	assert.InDelta(t, 2200.0, p.TotalCost, delta)
	assert.InDelta(t, 110.0, p.AvgCost, delta)
}
...
func TestRemoveQuantity_PartialSell(t *testing.T) {
	p := newPosition()
	p.AddQuantity(10, 100.0) // avgCost = 100, totalCost = 1000

	realized := p.RemoveQuantity(4, 150.0) // sell 4 @ 150 → proceeds 600, costBasis 400

	assert.InDelta(t, 200.0, realized, delta) // 600 - 400
	assert.InDelta(t, 6.0, p.Quantity, delta)
	assert.InDelta(t, 200.0, p.RealizedPnL, delta)
...
func TestRemoveQuantity_FullSell_ClosePosition(t *testing.T) {
	...
	p.RemoveQuantity(10, 120.0)

	assert.InDelta(t, 0.0, p.Quantity, delta)
...
func TestRemoveQuantity_SellMoreThanAvailable_CappedToAvailable(t *testing.T) {
	p := newPosition()
	p.AddQuantity(5, 100.0)

	realized := p.RemoveQuantity(20, 120.0) // only 5 available

	assert.InDelta(t, 0.0, p.Quantity, delta)
	assert.Equal(t, "closed", p.Status)
	// realized = 5 * 120 - 5 * 100 = 600 - 500 = 100
	assert.InDelta(t, 100.0, realized, delta)
}
...
func TestRemoveQuantity_ReturnsCorrectRealizedPnL(t *testing.T) {
	p := newPosition()
	p.AddQuantity(10, 80.0) // avgCost = 80, totalCost = 800

	realized := p.RemoveQuantity(5, 100.0) // proceeds = 500, costBasis = 400
...
func TestCalculateMetrics_CurrentValue(t *testing.T) {
	p := newPosition()
	p.AddQuantity(10, 100.0)
	p.CurrentPrice = 130.0
	p.CalculateMetrics()

	assert.InDelta(t, 1300.0, p.CurrentValue, delta) // 10 * 130
}

// 9. CalculateMetrics(): UnrealizedPnL = CurrentValue - TotalCost.
func TestCalculateMetrics_UnrealizedPnL(t *testing.T) {
	p := newPosition()
	p.AddQuantity(10, 100.0) // TotalCost = 1000
	p.CurrentPrice = 130.0
	p.CalculateMetrics()

	assert.InDelta(t, 300.0, p.UnrealizedPnL, delta) // 1300 - 1000
}
